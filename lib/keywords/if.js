module.exports = {
  ...
  compile(gctx, sctx, schema) {
    ...
    this.compile(gctx, schema, 'if');

    // compile then subschema
    if ('then' in schema) {
      variant += 1;
      this.compile(gctx, schema, 'then');
    }

    // compile else subschema
    if ('else' in schema) {
      variant += 2;
      this.compile(gctx, schema, 'else');
    }
...
function validateIfThen(gctx, sctx, dctx, schema, data) {
  try {
    this.validate(gctx, schema.if, data);
  ...
  // then condition
  this.validate(gctx, schema.then, data);
}

function validateIfElse(gctx, sctx, dctx, schema, data) {
  try {
    this.validate(gctx, schema.if, data);

  } catch (err) {
    // else condition
    this.validate(gctx, schema.else, data);
  }
}

function validateIfThenElse(gctx, sctx, dctx, schema, data) {
  try {
    this.validate(gctx, schema.if, data);
