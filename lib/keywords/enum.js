...
module.exports = {
  ...
  compile(rctx, sctx, schema) {
    ...
    // define enum map
    sctx.ENUM_STRING_MAP = new Map();

    // set enum strings
    for (let i = 0, l = schema.enum.length; i < l; i++) {
      sctx.ENUM_STRING_MAP.set(stringify(schema.enum[i]), true);
    }
...
function validateEnumMap(rctx, sctx, dctx, schema, data) {
  if (!sctx.ENUM_STRING_MAP.has(dctx.DATA_STRING)) {
