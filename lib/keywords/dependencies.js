  ...
  compile(rctx, sctx, schema) {
    ...
    for (const dependentProperty of dependentProperties) {
      ...
      if (Array.isArray(schema.dependencies[dependentProperty])) {
        ...
        this.compile(rctx, schema.dependencies, dependentProperty);
      }
    }
...
function validateDependencies(rctx, sctx, dctx, schema, data, property) {
  ...
  switch (dependencyType) {
    ...
    case 2:
      // schema dependency
      this.validate(rctx, schema.dependencies[property], data);
      break;
  }
}
