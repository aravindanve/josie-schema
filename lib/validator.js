...
const self = module.exports = {
  validate(sctx, fragment, data) {
    // handle boolean fragments
    if (fragment === false) {
      throw 'false error';
    }
    if (fragment === true) {
    ...
    const fctx = context.getFragmentContext(sctx, fragment);
    ...
    if (fctx.DEFINE_DATA_STRING) {
      dctx.DATA_STRING = stringify(data);
    }

    // get validators
    const validators = fctx.GET_VALIDATORS_FOR_TYPE[dctx.DATA_TYPE];

    // iterate through validators in queue
    for (const validator of validators) {
      validator.call(self, sctx, fctx, dctx, fragment, data);
    }
    ...
    // get loop validators
    const loopValidators = fctx.GET_LOOP_VALIDATORS_FOR_TYPE[dctx.DATA_TYPE];
    ...
    if (dctx.DATA_KEYS) {
      ...
      for (const key of dctx.DATA_KEYS) {
        ...
        for (const validator of loopValidators) {
          validator.call(self, sctx, fctx, dctx, fragment, data, key);
        }
      }

    } else {
      // define item string map if required
      if (fctx.DEFINE_DATA_ITEM_STRING_MAP) {
      ...
      for (let i = 0, l = dctx.DATA_LENGTH; i < l; i++) {
        // iterate through in-loop validators in queue
        for (const validator of loopValidators) {
          validator.call(self, sctx, fctx, dctx, fragment, data, i);
        }
      }
    }
  }
