const context = require('./keywords/context');
const stringify = require('../utils/stringify');

const self = module.exports = {
  validate(gctx, schema, data) {
    ...
    // get schema context
    const sctx = context.get(gctx, schema);
    ...
    for (const validator of validators) {
      validator.call(self, gctx, sctx, dctx, schema, data);
    }
    ...
    if (dctx.DATA_KEYS) {
      ...
      for (const key of dctx.DATA_KEYS) {
        ...
        for (const validator of loopValidators) {
          validator.call(self, gctx, sctx, dctx, schema, data, key);
        }
      }
      ...
      for (let i = 0, l = dctx.DATA_LENGTH; i < l; i++) {
        // iterate through in-loop validators in queue
        for (const validator of loopValidators) {
          validator.call(self, gctx, sctx, dctx, schema, data, i);
        }
      }
    }
  }
